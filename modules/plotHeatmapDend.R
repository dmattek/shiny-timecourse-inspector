...
## UI ----
modPlotHMdendUI <- function(id, label = "Hierarchical Clustering") {
  ns <- NS(id)
...
## SERVER ----
modPlotHMdend <- function(input, output, session, 
                          inDataWide, 
                          inDend,
                          inMeth) {
  ...
  plotHier <- function() {
    cat(file = stderr(), 'modPlotHMdend:plotHier: in\n')
